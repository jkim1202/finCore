...
| `email` | 사용자 이메일. 중복 불가 |
| `phone` | 사용자 연락처. 중복 불가 |
| `user_status` | 사용자 상태. `ACTIVE`, `INACTIVE` |
| `birth_date` | 생년월일 |
| `password_hash` | 인증에 사용할 비밀번호 해시 |
| `created_at` | 사용자 등록 시각 |

## user_roles

사용자가 가진 권한 목록이다. 한 사용자는 여러 역할을 가질 수 있다.

| Column | Meaning |
| --- | --- |
| `user_id` | 역할을 가진 사용자 |
| `role` | 사용자 권한. `CUSTOMER`, `ADMIN` |

## account

사용자 명의의 계좌다. 현재 잔액은 이 테이블에 있고, 거래 이력은 `account_transaction`에 남긴다.
...
사용자가 제출한 대출 신청서다. 심사 전후 상태를 갖고, 실행 계좌도 여기서 지정한다.
...
- `CUSTOMER`: 일반 금융 사용자
- `ADMIN`: 심사, 운영, 내부 관리 사용자

### users.user_status

- `ACTIVE`: 정상 사용 가능한 사용자
- `INACTIVE`: 비활성화된 사용자
